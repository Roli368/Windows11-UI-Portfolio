...
export default function DesktopIcon({ label, onDoubleClick }) {
  // Logic to determine icon style/color based on file extension
  const getFileStyle = (fileName) => {
    if (fileName.endsWith(".py")) return { color: "text-blue-400", bg: "bg-blue-500/15", border: "border-blue-500/30", hasShortcut: false };
    if (fileName.endsWith(".pdf")) return { color: "text-red-500", bg: "bg-red-500/15", border: "border-red-500/30", hasShortcut: false };
    if (fileName.endsWith(".exe") || fileName.endsWith(".sh")) return { color: "text-indigo-400", bg: "bg-indigo-500/15", border: "border-indigo-500/30", hasShortcut: true };
    if (fileName.endsWith(".log") || fileName.endsWith(".api")) return { color: "text-emerald-400", bg: "bg-emerald-500/15", border: "border-emerald-500/30", hasShortcut: true };
    
    // Default Folder style
    return { color: "text-yellow-500", bg: "bg-yellow-500/10", border: "border-yellow-500/20", hasShortcut: false };
  };
  ...
  return (
    <motion.div
      ...
      className="group flex flex-col items-center gap-2 cursor-pointer select-none w-24 py-2"
    >
      <div className={`relative w-16 h-16 flex items-center justify-center rounded-2xl 
        ...
        {style.hasShortcut && (
          <div className="absolute bottom-1 left-1 w-4 h-4 bg-white/90 rounded-sm flex items-center justify-center shadow-sm">
      ...
      <span className="text-[11px] font-medium text-slate-200 text-center px-2 py-0.5 
  ...
  return (
    ...
    >
      <div className={`relative w-16 h-16 flex items-center justify-center rounded-2xl 
        ...
        {style.isShortcut && (
          <div className="absolute bottom-1.5 left-1.5 w-4 h-4 bg-white/90 rounded-sm flex items-center justify-center shadow-sm border border-black/10">
            <svg viewBox="0 0 24 24" width="10" height="10" fill="black">
              <path d="M21 7l-4.5 4.5 1.5 1.5H12v-6l1.5 1.5L18 3l3 4zM5 21h14v-8H5v8z" />
            </svg>
          </div>
