...
export default function Desktop() {
  ...
  const startSelection = useCallback((e) => {
    // Only allow selection rectangle on larger screens where mouse is used
  ...
  return (
    <div
      ...
      onContextMenu={(e) => { 
        if (window.innerWidth > 768) { // Disable custom context menu on mobile
          e.preventDefault(); 
    ...
    >
      ...
      <div className="absolute top-4 left-4 sm:top-8 sm:left-8 grid grid-cols-4 sm:grid-cols-1 md:grid-flow-col md:grid-rows-6 gap-y-4 gap-x-2 sm:gap-y-6 sm:gap-x-4 w-fit z-10">
        {apps.map((app) => (
          <DesktopIcon 
            key={app.id}
            label={app.label} 
            // Standardizing for touch and mouse
        ...
        {apps.map((app) => {
          ...
          return isVisible && (
            <Window 
              ...
              isActive={isActive}
              // Stack windows on mobile, layer on desktop
...
export default function Desktop() {
  ...
  return (
    ...
    >
      ...
      {/* 2. Responsive Icon Grid Update
          Changed md:grid-rows-6 to dynamic repeat(auto-fill, 110px) 
          and added h-full to prevent icon overflow issues.
      */}
