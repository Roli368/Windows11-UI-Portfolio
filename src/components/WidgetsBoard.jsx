...
export default function WidgetsBoard({ closeBoard }) {
  ...
  // Memoize fetchWeather to avoid unnecessary re-renders in useEffect
  ...
  return (
    <motion.div
      ...
      exit={{ x: -500, opacity: 0 }}
      transition={{ type: "spring", damping: 25, stiffness: 200 }}
      // Responsive width: nearly full-screen on mobile, fixed on desktop
      // Dynamic height: adjusts for mobile browser UI
      className="fixed top-2 left-2 h-[calc(100dvh-80px)] w-[95vw] sm:w-[520px] bg-[#0f172a]/75 backdrop-blur-3xl border border-white/10 rounded-2xl shadow-2xl z-[10000] overflow-y-auto no-scrollbar p-4 sm:p-6 select-none"
    >
      <div className="flex justify-between items-center mb-8">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-full bg-indigo-600 flex items-center justify-center text-white text-xs font-bold shadow-lg">R</div>
        ...
        <motion.button 
          whileTap={{ scale: 0.9 }} // Visual feedback for mobile tap
          onClick={closeBoard} 
      ...
      {error ? (
        ...
        <div className="bg-gradient-to-br from-indigo-500/30 to-blue-600/30 border border-white/10 p-6 rounded-2xl mb-6 shadow-inner relative overflow-hidden">
          ...
          <div className="flex justify-between items-center">
            <div>
              <p className="text-4xl sm:text-5xl font-bold text-white drop-shadow-md">{weather.temp}°C</p>
              <p className="text-[10px] sm:text-xs text-slate-200 font-medium capitalize mt-1">
                {weather.desc} • {weather.city}
              </p>
            </div>
            <span className="text-5xl sm:text-6xl drop-shadow-2xl">{weather.icon}</span>
          </div>
        </div>
      )}

      {/* Adaptive Grid: 1 column on mobile, 2 on tablet/laptop */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pb-4">
        ...
        <div className="bg-slate-900/50 p-5 rounded-2xl border border-white/5 hover:border-indigo-500/30 transition-all group">
          ...
          <p className="text-[11px] text-slate-400"><span className="text-white font-bold">12 commits</span> this week</p>
        </div>
        ...
        <div className="bg-slate-900/50 p-5 rounded-2xl border border-white/5 hover:border-indigo-500/30 transition-all">
          ...
          <div className="space-y-3">
            <div className="border-l-2 border-indigo-500 pl-3">
              <p className="text-[11px] text-white font-medium line-clamp-1">AI Agents redefine SaaS workflows</p>
              <p className="text-[9px] text-slate-500 mt-0.5">24m ago</p>
            </div>
            <div className="border-l-2 border-slate-700 pl-3">
              <p className="text-[11px] text-white font-medium line-clamp-1">React 19 stable release features</p>
              <p className="text-[9px] text-slate-500 mt-0.5">2h ago</p>
            </div>
          </div>
        </div>

        {/* System Status Card */}
        <div className="sm:col-span-2 bg-[#1e293b]/30 p-4 rounded-xl border border-white/5 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <motion.button 
            whileTap={{ scale: 0.95 }}
            className="w-full sm:w-auto text-[10px] bg-white/5 hover:bg-white/10 px-4 py-2 rounded-lg border border-white/10 text-white transition-all"
          >
            System Logs
          </motion.button>
        </div>
